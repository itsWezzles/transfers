...
def set_ghost_points(u, comm_col, comm_row):
  ...
  # Create buffer for Irecv top and bottom
  top_recv = np.ascontiguousarray(u[0,1:Iy])
  bot_recv = np.ascontiguousarray(u[Ix,1:Iy])

  # Within each column, send the bottom to become the top of the next with Isend/Irecv, post recv, send, then wait for status

  recvReq = comm_col.Irecv(top_recv, source=(p_row-1)%max_row)
  comm_col.Isend(u[Ix-1,1:Iy], dest=(p_row+1)%max_row)
  status = MPI.Status()
  recvReq.Wait(status)

  # Within each column, send the top to become the bottom of the previous with Isend/Irecv

  recvReq = comm_col.Irecv(bot_recv, source=(p_row+1)%max_row)
  comm_col.Isend(u[1,1:Iy], dest=(p_row-1)%max_row)
  status = MPI.Status()
  recvReq.Wait(status)

  #Place back values from Irecv
  u[0,1:Iy] = top_recv
  u[Ix,1:Iy] = bot_recv

  # Create buffer for Irecv left and right
  left_recv = np.ascontiguousarray(u[1:Ix,0])
  right_recv = np.ascontiguousarray(u[1:Ix,Iy])

  # Within each row, send the right to become the left of the next using Isend/Irecv

  recvReq = comm_row.Irecv(left_recv, source=(p_col-1)%max_col)
  comm_row.Isend(np.ascontiguousarray(u[1:Ix,Iy-1]), dest=(p_col+1)%max_col)
  status = MPI.Status()
  recvReq.Wait(status)

  # Within each row, send the left to become the right of the previous using Isend/Irecv

  recvReq = comm_row.Irecv(right_recv, source=(p_col+1)%max_col)
  comm_row.Isend(np.ascontiguousarray(u[1:Ix,1]), dest=(p_col-1)%max_col)
  status = MPI.Status()
  recvReq.Wait(status)
